...
export class MinuteEventEmitter extends EventEmitter {
    ...
    private setupInitialTimeout() {
        ...
        //3sec buffer is given to avoid any corner cases where time is exactly at the starting minute. For ex: 5:31:00 AM
        const msUntilNextEvent = nextEventTime.getTime() - now.getTime() + 3000;
    ...
    private calculateNextEventTime(currentTime: Date): Date {
        ...
        const elapsedTime = (currentTime.getTime() - startPoint.getTime()) / 1000;
        const elapsedSeconds = Math.floor(elapsedTime);
        const nextEventSeconds = Math.ceil(elapsedSeconds / intervalSeconds) * intervalSeconds;
        const nextEventTime = new Date(startPoint.getTime() + nextEventSeconds * 1000);
